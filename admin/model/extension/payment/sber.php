...
class ModelExtensionPaymentSber extends Model {
	public function install() {
		$this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "sber_order` (
	...
	public function deleteTables() {
		$this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "sber_order`");
	}

	public function getGatewayOrder($order_id) {
		$result = $this->db->query("SELECT * FROM `" . DB_PREFIX . "sber_order` WHERE order_id = " . (int)$order_id . " LIMIT 1");

		return $result->row;
	}

	public function updateGatewayOrder($order_id, $data) {
		$sql = "UPDATE `" . DB_PREFIX . "sber_order` SET ";
