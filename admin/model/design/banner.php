		...
		if (isset($data['banner_image'])) {
			foreach ($data['banner_image'] as $language_id => $value) {
				foreach ($value as $banner_image) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "banner_image SET banner_id = '" . (int)$banner_id . "', language_id = '" . (int)$language_id . "', title = '" .  $this->db->escape($banner_image['title']) . "', link = '" .  $this->db->escape($banner_image['link']) . "', link_mob = '" .  $this->db->escape($banner_image['link_mob']) . "', image = '" .  $this->db->escape($banner_image['image']) . "', btn_link = '" . $banner_image['btn_link'] . "', sort_order = '" .  (int)$banner_image['sort_order'] . "'");
				}
			}
		}
	...
	public function getBannerImages($banner_id) {
		...
		foreach ($banner_image_query->rows as $banner_image) {
			$banner_image_data[$banner_image['language_id']][] = array(
				...
				'link_mob'   => $banner_image['link_mob'],
				'image'      => $banner_image['image'],
				'btn_link'   => $banner_image['btn_link'],
