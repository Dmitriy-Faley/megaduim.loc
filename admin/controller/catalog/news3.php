<?php
class ControllerCatalogNews3 extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('catalog/news3');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_news3->addNews($this->request->post);
			...
			$this->response->redirect($this->url->link('catalog/news3', 'user_token=' . $this->session->data['user_token'] . $url, true));
		}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_news3->editNews($this->request->get['news3_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $news3_id) {
				$this->model_catalog_news3->deleteNews($news3_id);
			}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('catalog/news3', 'user_token=' . $this->session->data['user_token'] . $url, true)
		);

		$data['add'] = $this->url->link('catalog/news3/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['delete'] = $this->url->link('catalog/news3/delete', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$news_total = $this->model_catalog_news3->getTotalNews();

		$results = $this->model_catalog_news3->getNews($filter_data);

		foreach ($results as $result) {
			$data['news'][] = array(
				'news3_id' => $result['news3_id'],
				'title' => $result['title'],
				'sort_order' => $result['sort_order'],
				'edit' => $this->url->link('catalog/news3/edit', 'user_token=' . $this->session->data['user_token'] . '&news3_id=' . $result['news3_id'] . $url, true)
			);
		}
		...
		$data['sort_title'] = $this->url->link('catalog/news3', 'user_token=' . $this->session->data['user_token'] . '&sort=id.title' . $url, true);
		$data['sort_sort_order'] = $this->url->link('catalog/news3', 'user_token=' . $this->session->data['user_token'] . '&sort=i.sort_order' . $url, true);
		...
		$pagination->url = $this->url->link('catalog/news3', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('catalog/news_list3', $data));
	}

	protected function getForm() {
		$data['text_form'] = !isset($this->request->get['news3_id']) ? $this->language->get('text_add') : $this->language->get('text_edit');
		...
		if (!isset($this->request->get['news3_id'])) {
			$data['action'] = $this->url->link('catalog/news3/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('catalog/news3/edit', 'user_token=' . $this->session->data['user_token'] . '&news3_id=' . $this->request->get['news3_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('catalog/news3', 'user_token=' . $this->session->data['user_token'] . $url, true);

		if (isset($this->request->get['news3_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$news_info = $this->model_catalog_news3->getNewsItem($this->request->get['news3_id']);
		}
		...
		if (isset($this->request->post['news_description'])) {
			...
			$data['news_description'] = $this->model_catalog_news3->getNewsDescriptions($this->request->get['news3_id']);
		} else {
		...
		if (isset($this->request->post['news_store'])) {
			...
			$data['news_store'] = $this->model_catalog_news3->getNewsStores($this->request->get['news3_id']);
		} else {
		...
		if (isset($this->request->post['news3_seo_url'])) {
			$data['news3_seo_url'] = $this->request->post['news3_seo_url'];
		} elseif (isset($this->request->get['news3_id'])) {
			$data['news3_seo_url'] = $this->model_catalog_news3->getNewsSeoUrls($this->request->get['news3_id']);
		} else {
			$data['news3_seo_url'] = array();
		}

		if (isset($this->request->post['news_layout'])) {
			$data['news_layout'] = $this->request->post['news_layout'];
		} elseif (isset($this->request->get['news3_id'])) {
			$data['news_layout'] = $this->model_catalog_news3->getNewsLayouts($this->request->get['news3_id']);
		} else {
		...
		$this->response->setOutput($this->load->view('catalog/news_form3', $data));
	}

	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'catalog/news3')) {
		...
		if ($this->request->post['news3_seo_url']) {
			$this->load->model('design/seo_url');

			foreach ($this->request->post['news3_seo_url'] as $store_id => $language) {
				foreach ($language as $language_id => $keyword) {
					if (!empty($keyword)) {
						...
						foreach ($seo_urls as $seo_url) {
							if (($seo_url['store_id'] == $store_id) && (!isset($this->request->get['news3_id']) || ($seo_url['query'] != 'news3_id=' . $this->request->get['news3_id']))) {
