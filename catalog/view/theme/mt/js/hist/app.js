(function () {
  ...
  */
  ...
  let ModalDispatcher = (function () {
    ...
    //Удаление data атрибута linked-modal у товаров на разрешении менье 750
    if (window.matchMedia("(max-width:750px)").matches) {
      const items = document.querySelectorAll('.hits_slide__pic');
      items.forEach(element => {
        delete element.dataset.linkedModal;
      });
    }
  	...
  	function bindEvents() {
         ...
         // let btnCard = event.target.closest('.hits_slide__right')
  ...
  function activeLink () {
      ...
      let linkArray = location.href.split('');

      activeLinks.forEach(el => {
          let theLink = el.href
          if(linkArray[linkArray.length - 1] == '/') {
            if (theLink + '/' === location.href) {
          ...
          ymaps.ready(function () {
              var myMap = new ymaps.Map('modalMap', {
                  ...
                  myPlacemark = new ymaps.Placemark(myMap.getCenter(), {
                      ...
                      iconImageHref: '/image/catalog/assets/img/map_marker.svg',
                      // Размеры метки.
                      iconImageSize: [42, 53],
