<?php
class ControllerInformationNews3 extends Controller {

	public function index() {
		$this->load->language('information/news3');
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('information/news3'),
		...
		if ($news) {
			...
			foreach ($news as $result) {
				...
				$data['news'][] = array(
					...
					'href' => $this->url->link('information/news3/info', 'news3_id=' . $result['news3_id']),
			...
			$this->response->setOutput($this->load->view('information/news_list3', $data));
		} else {
	...
	public function info() {
		...
		if (isset($this->request->get['news3_id'])) {
			$news3_id = (int) $this->request->get['news3_id'];
		} else {
			$news3_id = 0;
		}
		...
		$this->load->model('tool/image');
		$news = $this->model_catalog_news3->getNewsItem($news3_id);
		...
		if ($news) {
			...
			$data['breadcrumbs'][] = array(
				'text' => $news['title'],
				'href' => $this->url->link('information/news3/info', 'news3_id=' . $news3_id),
				'separator' => $this->language->get('text_separator')
			);
			...
			$this->response->setOutput($this->load->view('information/news_info3', $data));
		} else {
			$data['breadcrumbs'][] = array(
