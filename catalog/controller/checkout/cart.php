...
class ControllerCheckoutCart extends Controller {
	public function index() {
		...
		foreach ($results as $result) {
			...
			foreach ($results as $resultImage) {
				$data['images'][] = array(
					'thumb' => $this->model_tool_image->resize($resultImage['image'], $this->config->get('theme_' . $this->config->get('config_theme') . '_image_additional_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_additional_height'))
				);
			}
		...
		if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			$specialAll = 0; // Счетчик общей скидки
			foreach ($products as $product) {
				...
				if ((float)$product['special']) {
					$special = $this->currency->format($this->tax->calculate($product['special'], $product['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency'], '', false);
					$specialCount = $this->currency->format($this->tax->calculate($product['specialCount'], $product['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency'], '', false);
					$specialAll += $product['total'];
					$specialAll = $specialAll -= $product['specialCount'];
				} else {
					$special = false;
					$specialCount = false;
				}
				...
				$data['in_wishlist'] = (in_array($product['product_id'], array_column($wishlist, 'product_id')) ? 1 : 0);
				...
				$data['products'][] = array(
					...
					'specialCount'   => $specialCount,
					'total'     => $total,
					'normalCount'     => (float)$product['normal'] * $product['quantity'],
					'normal'     => (float)$product['normal'],
			...
			foreach ($totals as $total) {
				$data['totals'][] = array(
					'title' => $total['title'],
					'text'  => $this->currency->format($total['value'] - $specialAll, $this->session->data['currency']),
					'sale'  => $specialAll,
					'totalSale'  => $this->currency->format($total['value'], $this->session->data['currency']) - $specialAll,
				);
			}
